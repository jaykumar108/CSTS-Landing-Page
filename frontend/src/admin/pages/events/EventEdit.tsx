    ...
    const fetchEvent = async () => {
      try {
        ...
        if (eventData.imageUrl) {
          setCurrentImage(eventData.imageUrl);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const eventData = {
        ...formData,
        imageUrl,
        cloudinaryId: cloudinaryId || undefined // Only include if there's a new value
      };
      
      // Update event with authorization header
      const config = {
        headers: {
                ...
                <div className="mt-1 flex justify-center px-6 pt-5 pb-6 border-2 border-gray-300 border-dashed rounded-md">
                  <div className="space-y-1 text-center">
                    ...
                    <div className="flex text-sm text-gray-600">
                      ...
                      >
                        <span>{currentImage ? 'Change image' : 'Upload an image'}</span>
                        <input
                          id="file-upload"
                          name="file-upload"
                          type="file"
